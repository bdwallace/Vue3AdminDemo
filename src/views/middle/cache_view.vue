    ...
    <div class="main">
      ...
      <div class="redisTable">
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" v-loading="loading"
          ...
          <el-table-column prop="cache_name" label="名称" fit align="center">
            <template #default="{ row }">
              <el-button type="primary" text size="small" :icon="CopyDocument" v-if="row.cache_name"
                         @click="copyText(row.cache_name)" style="margin-right: 2px"></el-button>
              <el-tooltip class="item" effect="dark" :content="row.cache_name" placement="top">
                <b style="color: #00a1c9">{{ row.cache_name }}</b>
              </el-tooltip>
            </template>
          </el-table-column>
      ...
      <el-dialog title="创建MSK" v-model="addDialogVisable" width="45%">
...
async function fetchData() {
  loading.value = true
  var resp = await getRedisData(params).catch(() => {
    ElMessage({type: 'error', message: '请求响应错误，请联系管理员'})
    return 0
  })
  if (resp.code === 200) {
